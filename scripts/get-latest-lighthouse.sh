...
printf "NOTE: Assumes target binary/arch is x86_64-unknown-linux-gnu-portable.tar.gz\n"
...
| jq -r '.assets[] | select(.name |test("x86_64-unknown-linux-gnu-portable.tar.gz$")) .browser_download_url')"

# get the latest release binary url .asc file
LATEST_LIGHTHOUSE_ASC_URL="$(curl -s $SIGP_LIGHTHOUSE_RELEASE_URL \
| jq -r '.assets[] | select(.name |test("x86_64-unknown-linux-gnu-portable.tar.gz.asc$")) .browser_download_url')"
